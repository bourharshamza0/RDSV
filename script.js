...
async function createLetter() {
    ...
    if (success) {
        ...
        content = content.replace(/{{module}}/g, userData.date);
...
function openPrintWindow(content, lang) {
    const printWindow = window.open('', '_blank'); 
        ...
        <style>
            /* ضبط إعدادات الورقة عند الطباعة */
            @page {
            size: A4 portrait;
            margin: 0; /* تحكم بالهوامش عن طريق الكود داخلياً */
            }
            ...
            * {
            ...
            margin: 0; 
            padding: 0; 
            background-color: #f5f5f5; /* لون خلفية خفيف للمعاينة فقط */
            }

            /* حاوية الصفحة بحجم A4 الحقيقي */
            .a4-page {
            width: 210mm;
            min-height: 297mm;
            padding: 20mm; /* الهوامش الداخلية للنص */
            margin: 10px auto;
            background: #ffffff;
            ...
            /* إخفاء العناصر غير المخصصة للطباعة */
            @media print { 
            body { 
            ...
            /* تنسيقات المراسلة باللغة الفرنسية */
            .letter-headerfr { 
                                font-size: 17px; 
                                font-weight: bold; 
                                margin-bottom: 10px; 
...
function addTableClickEvent() {
    document.addEventListener('click', function(e) {
        ...
        if (document.getElementById('module')) document.getElementById('module').value = cells[0].textContent.trim();
        if (document.getElementById('date')) document.getElementById('date').value = cells[1].textContent.trim();
        if (document.getElementById('demande')) document.getElementById('demande').value = cells[2].textContent.trim();
        if (document.getElementById('adresse')) document.getElementById('adresse').value = cells[3].textContent.trim();
        if (document.getElementById('ville')) document.getElementById('ville').value = cells[4].textContent.trim(); // حقل المدينة
        if (document.getElementById('nom')) document.getElementById('nom').value = cells[5].textContent.trim();
        if (document.getElementById('objet')) document.getElementById('objet').value = cells[6].textContent.trim();
